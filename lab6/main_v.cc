...
bool is_valid(std::string const& str)
{
	...
	// words not starting/ending with a '-' are not valid
	...
	return std::all_of(str.begin(), str.end(), [&swe_loc](char c){return isalpha(c, swe_loc) || c == '-';});
}
...
int main(int argc, char* argv[])
{
	...
	else if (argc < 3 || argv[2][0] != '-' || (argv[2][1] != 'a' && argv[2][1] != 'f' && argv[2][1] != 'o'))
	{
	...
	std::transform(words.begin(), words.end(), words.begin(), [](std::string & str) { return clean_word(str); });

	// validate clean words
	auto it = std::remove_if(words.begin(), words.end(), [](std::string const& str) {return !is_valid(str);} );
	...
	else if (argv[2][1] == 'f')
	{
		...
		std::transform(unique_words.begin(), unique_words.end(), std::back_inserter(word_count), 
					   [&words](std::string & str) { return std::count(words.begin(), words.end(), str); });
		
		// merge into a vector of pairs
		std::vector<std::pair<std::string, int>> pairs;
		std::transform(unique_words.begin(), unique_words.end(), word_count.begin(), std::back_inserter(pairs),
               [](std::string & str, int cnt) { return std::make_pair(str, cnt); });


		// sort vector<string, int> by int 
		std::sort(pairs.begin(), pairs.end(), [](std::pair<std::string,int> l, \
			std::pair<std::string,int> r) { return l.second > r.second; }); 

		// print
		std::transform(pairs.begin(), pairs.end(), std::ostream_iterator<std::string>(std::cout, "\n" ), 
					   [](std::pair<std::string, int> & p) { return p.first + " " + std::to_string(p.second); } );
	...
	else if (argv[2][1] == 'o')
	{
		...
		{
			if (length != 0 && length + str.length() + 1 >= max_length)
			{
				str_stream << "\n";
				length = 0;
			}
